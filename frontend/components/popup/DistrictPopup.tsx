  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-slate-900
      ...
      <div className="hidden sm:flex items-center justify-between px-4 py-3
        ...
        <div>
          <div className="font-bold text-slate-900 dark:text-slate-100">{district.district}</div>
          <div className="text-xs text-slate-500 dark:text-slate-400">{district.region}</div>
        </div>
      ...
      {/* Spike counts */}
      <div className="grid grid-cols-3 gap-2 px-4 py-3 border-b border-slate-200 dark:border-slate-700">
        {[
          { label: "Critical", value: district.critical_count, color: "text-red-500 dark:text-red-400"    },
          { label: "Severe",   value: district.severe_count,   color: "text-orange-500 dark:text-orange-400" },
          { label: "Moderate", value: district.moderate_count, color: "text-yellow-500 dark:text-yellow-400" },
        ].map(s => (
      ...
      <div className="px-4 py-3 border-b border-slate-200 dark:border-slate-700 space-y-1">
        ...
        <div className="flex justify-between text-xs">
          <span className="text-slate-500 dark:text-slate-400">Spike rate</span>
          <span className="font-mono text-slate-800 dark:text-slate-200">{district.spike_rate_pct}%</span>
        </div>
        <div className="flex justify-between text-xs">
          <span className="text-slate-500 dark:text-slate-400">Markets monitored</span>
          <span className="font-mono text-slate-800 dark:text-slate-200">{district.markets.length}</span>
        </div>
      </div>
      ...
      <div className="px-4 py-2 border-b border-slate-200 dark:border-slate-700">
        <button
          ...
          className="w-full flex items-center justify-center gap-2 text-xs py-2
                     rounded border border-yellow-500/50 text-yellow-600 dark:text-yellow-400
                     hover:bg-yellow-50 dark:hover:bg-yellow-500/10 transition-colors font-medium"
        >
        ...
        <div className="space-y-1">
          {district.markets.map((m, i) => (
            <div key={i} className="flex justify-between text-xs">
              <span className="text-slate-700 dark:text-slate-300">{m.market}</span>
              <span className="text-slate-400 dark:text-slate-500">{m.spike_rate_pct.toFixed(1)}%</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="px-4 py-3">
        ...
        <div className="space-y-2">
          {district.recent_critical_spikes.length === 0 && (
            <div className="text-xs text-slate-400 dark:text-slate-600">No critical spikes</div>
          )}
          {district.recent_critical_spikes.map((s, i) => (
            <div key={i} className="bg-slate-100 dark:bg-slate-800 rounded p-2">
              <div className="flex justify-between text-xs">
                <span className="font-medium text-slate-800 dark:text-slate-200">{s.commodity}</span>
                <span className="font-bold text-red-500 dark:text-red-400">+{s.pct_change}%</span>
              </div>
